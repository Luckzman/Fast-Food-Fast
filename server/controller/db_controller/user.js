...
export const createDeliveryInfo = (req, res) => {
  ...
  db.query(query, values)
    .then(info => responseMsg(res, 200, 'success', 'Delivery successfully added', info.rows[0]))
...
export const updateDeliveryInfo = (req, res) => {
  ...
  const values = [
    address, city, state, new Date(), req.params.id,
  ];
  db.query(query, values)
    .then(info => responseMsg(res, 200, 'success', 'Delivery successfully added', info.rows[0]))
    .catch(error => res.status(400).json(error));
...
export const getUsers = (req, res) => {
  const query = 'SELECT users.id, firstname, lastname, email, phone, image, user_status, created_date FROM users';
  // INNER JOIN users_address ON users.id = users_address.user_id';
